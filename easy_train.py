...
def get_label(file_path):
  ...
  return a
def decode_img(img,IMG_WIDTH=17,IMG_HEIGHT=30):
  # convert the compressed string to a 3D uint8 tensor
  img = tf.image.decode_jpeg(img, channels=3)
  return tf.image.resize(img, [IMG_WIDTH, IMG_HEIGHT])
def process_path(file_path):
  #取得label的熱鍵編碼
  label = get_label(file_path) #OneHotKey
...
#定義batchsize數量 ， batchsize 表示一次訓練的資料數
batch_size=20#64
#定義自動調整
...
labeled_ds_train = labeled_ds_train.cache().shuffle(total_train).map(rnd_pic,num_parallel_calls=AUTOTUNE).batch(batch_size).prefetch(buffer_size=AUTOTUNE)
...
model.compile(keras.optimizers.Adam(0.001),loss=keras.losses.CategoricalCrossentropy(),metrics=[keras.metrics.CategoricalAccuracy()])
